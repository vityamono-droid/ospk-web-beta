...
import NewsListItemSkeleton from './Skeleton'
...
const NewsListItem = ({ data, showBanner, showCategories }: NewsListItemProps) => {
  ...
  return (
    <>
      {!data ? (
        <NewsListItemSkeleton showBanner={showBanner} showCategories={showCategories} />
      ) : (
        <Paper
          variant={'outlined'}
          ...
          <Stack p={2} direction={'row'} spacing={2}>
            ...
            {showBanner && !!data.banner && (
              <Stack minWidth={150} sx={{ display: { xs: 'none', sm: 'flex' }, overflow: 'hidden', borderRadius: 1 }}>
                <img height={150} width={150} src={data.banner} />
              </Stack>
            )}
            {/* Content */}
            <Stack width={'100%'} spacing={2} sx={{ overflow: 'hidden' }}>
              {/* Title */}
