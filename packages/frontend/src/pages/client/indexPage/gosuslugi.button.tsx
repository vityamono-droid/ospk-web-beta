...
import GosuslugiModal from './gosuslugi.modal'
      ...
      <Paper elevation={0} sx={{ bgcolor: '#38bafe' }}>
        <Stack direction={{ xs: 'column', md: 'row' }} spacing={2}>
          ...
          <Stack p={2} width={'100%'} bgcolor={'white.main'} alignItems={'center'} justifyContent={'center'}>
            ...
            <Typography variant={'h5'}>Проще, чем кажется</Typography>
            <Typography>gosuslugi.ru</Typography>
          </Stack>
        </Stack>
      </Paper>
      {openModal && <GosuslugiModal open={openModal} onClose={() => setOpenModal(false)} />}
    </>
  )
}
